<script lang="ts">
	...
	async function updateUnconfirmedBank(
		...
		const newBankErg = bank_box_nano_erg + ergAddMem + ergAddPrep;
		const newBankUsd = bank_box_circulating_usd_cent + usdAddMem + usdAddPrep;
			...
			<div class="flex items-center gap-1 uppercase">
				{#if confirmed}
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="1em"
						...
						/></svg
					>
					{bank.type} @{oracleRateToUsd($oracle_price_sig_usd_cent)}
				{:else}
					<Spinner size={16} />
					{bank.type} @{oracleRateToUsd($oracle_price_sig_usd_cent)}
				{/if}
